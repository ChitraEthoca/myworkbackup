...
<ul id='scriptList'>
	<xsl:for-each select="//suite/test/class">
	 ...
	 <xsl:variable name="caseName" select="substring-after(@name, '.tests.')"></xsl:variable>
	<!-- <xsl:variable name="status"> <xsl:value-of select="./test-method/@status"/></xsl:variable> -->
	 <xsl:choose>
			  ...
			  <xsl:otherwise>
			   <a href="#testTable" onclick="deleteTableRowsAndRecreate('{$testCaseName}')"><li><xsl:value-of select='$caseName'/></li></a>
			  </xsl:otherwise>
			   </xsl:choose>
